import cv2 as cv  # Import the OpenCV library with an alias 'cv'
import math
import time
import argparse  # Import argparse to handle command-line arguments
import os

# Function to get the face bounding box using a pre-trained model
...
# Mean values for model input preprocessing
...
# Open a video file, an image file, or a camera stream based on the command-line argument
...
while cv.waitKey(1) < 0:
    ...
    for bbox in bboxes:
        ...
        # Construct the name for saving the image
        if args.i:  # If an input file path is provided
            name = os.path.basename(args.i)  # Extract the file name
        else:  # If using the webcam, use a default name
            name = 'webcam_capture.jpg'

        cv.imwrite(os.path.join('./detected', name), frameFace)  # Use os.path.join to handle path correctly
